...
public class ItemFrontRotationTool extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltipComponents, TooltipFlag isAdvanced) {
        ...
        tooltipComponents.add(new TranslatableComponent("item.structurelib.front_rotation_tool.desc.0")); // Triggers Front Rotation Interface
        // Rotates only the front panel,
        tooltipComponents.add(new TranslatableComponent("item.structurelib.front_rotation_tool.desc.1").withStyle(ChatFormatting.BLUE));
        // which allows structure rotation.
        tooltipComponents.add(new TranslatableComponent("item.structurelib.front_rotation_tool.desc.2").withStyle(ChatFormatting.BLUE));
    }
}
